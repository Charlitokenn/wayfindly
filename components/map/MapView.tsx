...
import { MapView as MappedinMapView, useMapData, useMap, Navigation } from "@mappedin/react-sdk";
...
// useMap() only works inside a <MapView> subtree, so this tiny bridge component
// renders as a child of MapView purely to surface { mapData, mapView } up to the
// parent once the SDK has actually finished initializing. MapView has no onLoad
// prop in @mappedin/react-sdk@6.21.1 -- this is the SDK's documented pattern.
function MapReadyBridge({ onReady }: { onReady: (mapView: any) => void }) {
  const { mapView } = useMap();

  useEffect(() => {
    if (mapView) {
      console.info("Mappedin map ready -> mapView set", mapView);
      onReady(mapView);
    }
  }, [mapView, onReady]);
...
export default function MapViewer({ mapId, booths, eventId }: MapViewerProps) {
  ...
  // Uses a callback ref (containerEl) instead of a plain useRef so this effect re-fires
  // exactly when the map container div actually mounts (e.g. once isLoading flips false
  // and we render the real JSX branch), instead of only running once on first mount when
  // the container may not exist in the DOM yet.
  useEffect(() => {
    if (!containerEl) return;

    const check = () => {
      const ok = containerEl.clientWidth > 0 && containerEl.clientHeight > 0;
      ...
      <div className="relative h-[calc(100vh-64px)] w-full overflow-hidden flex flex-col md:flex-row">
        ...
        <div ref={containerRef} className="w-full relative flex-1 min-h-0">
          {containerReady && mapData ? (
              ...
              >
                <MapReadyBridge onReady={(mv) => setMapView(mv)} />
