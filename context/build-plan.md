...
| # | Feature | Description |
|---|---------|-------------|
| 1 | Project setup | Next.js 16 + PWA (`next-pwa`) + Clerk (Google OAuth only) + InsForge + Tailwind v4 + shadcn/ui. Create full DB schema (venues, events, booths, leads, payments, user_profiles, user_visits, instagram_connections, contests, contest_entries, contest_audit_logs, promotions, promotion_entries, app_settings). Set up route groups: `(public)`, `(onboarding)`, `(attendee)`, `(admin)`, `(booth)`. Configure `clerkMiddleware()` in `middleware.ts`. |
| 2 | Google OAuth + Clerk routing | Clerk configured for Google OAuth only. `middleware.ts` enforces: unauthenticated → `(public)` only; incomplete onboarding → redirect `/onboarding`; org member → org-aware routing. `<UserProfile>` with personal profile enabled for org members. |
